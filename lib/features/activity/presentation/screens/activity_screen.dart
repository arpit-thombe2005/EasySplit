...
/// Activity Screen — timeline of settlement records only
class ActivityScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (settlementsAsync.isLoading && settlements.isEmpty) {
    ...
    if (settlements.isEmpty) {
      return Scaffold(
        ...
        body: const EmptyState(
          ...
          subtitle: 'Your settlement history will appear here once payments are recorded.',
        ),
      );
    }

    final pending = settlements.where((s) => s.status.toLowerCase() == 'pending').toList();
    final completed = settlements.where((s) => s.status.toLowerCase() != 'pending').toList();
        ...
        child: ListView(
          ...
          children: [
            ...
            if (completed.isNotEmpty) ...[
              Text(
                pending.isNotEmpty ? 'Past Settlements' : 'Settlement History',
                style: theme.textTheme.titleSmall?.copyWith(color: cs.secondary),
              ),
              const SizedBox(height: 12),
...
class _SettlementCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isRejected = settlement.status.toLowerCase() == 'rejected';
      ...
      child: Padding(
        ...
        child: Column(
          children: [
                      ...
                      Text(
                        settlement.settledAt != null
                            ...
                            : (settlement.createdAt != null
                                ? DateFormat('MMM d, yyyy').format(settlement.createdAt!)
                                : 'Pending'),
                ...
                Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        isCompleted ? 'Settled' : settlement.status.toUpperCase(),
                        style: theme.textTheme.labelSmall?.copyWith(
                          color: isCompleted
                              ? (isDark ? AppTheme.positiveBalanceDark : AppTheme.positiveBalance)
                              : (isRejected ? cs.onErrorContainer : cs.onSurface),
            ...
            if (!isCompleted && !isRejected && onSettle != null && !isOwer) ...[
              const SizedBox(height: 12),
              const Divider(),
